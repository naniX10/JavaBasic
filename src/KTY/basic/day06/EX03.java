...
public class EX03 {
    public static void main(String[] args) {
        ...
        letter = new char[3];   // 문자 3개를 저장할 수 있는 배열 크기 지정
        letter[0] = 'a';
        letter[1] = 'b';
        letter[2] = 'c';
        ...
        for(int i = 0; i < 3; ++i) {
            System.out.print(letter[i] + ","); // sout치고 탭!
        }
        ...
        a[1] = a[2];
        // 3번째 요소값을 2번째 요소에 대입입
       System.out.println(a[0] + " " + a[1] + " " + a[2]); // alt + 쉬프트 + 방향키!!
        ...
        for(int idx = 1; idx < sampleArray.length; ++idx) // = 빼면 된다!!!!
            sampleArray[idx] = 3*idx;
...
//            num[i-1] = i;     <= 이거나
        ...
        for(int i = 0; i < num2.length; i += 1){  // [] 안의 값을 지정
            num2[i] = i * 2;  // << 이거가 값 증가의 규칙인듯?
        ...
        for(int i = 0; i < num3.length; i += 1) {
            //num3[i] = (i + 1) * (i + 1);  // (i^2?) 아닌가바
        ...
        //for(int i = 0; i < num6.length; i += 1);
        //    System.out.print(num6[num6.length] + ", ");

        for(int i = (num6.length - 1); i >= 0 ; i -= 1) {
            ;
