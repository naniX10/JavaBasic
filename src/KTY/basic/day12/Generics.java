...
public class Generics {
    public static void main(String[] args) {
        ...
        // 단, 배열에서 객체를 빼낼 때는
        // 그 데이터 타입에 맞는 적절한 형 변환이 필요함
        ((Lion)zoo1[0]).sayName();
        ((Tiger)zoo1[1]).sayName();
        ((Zebra)zoo1[2]).sayName();

        // 배열에서 꺼낸 객체를 적절히 변환하려니 귀찮..
...
//        ((Tiger)zoo1[2]).sayName();
        ...
        // 오류발생 -> 비정상 형변환이 원인
        // 근데, 컴파일 할때는 아무런 오류가 없었음!
        // => Object 타입은 바로 정수로 변환 불가라서
        //System.out.println( Integer.parseInt(numbers2[0]) +
        //        Integer.parseInt(numbers2[1]) + Integer.parseInt(numbers2[2]) );

        // JDK 1.5 이전에서는 여러타입을 사용하는
        // 대부분의 클래스나 메서드에서 인수나 반환값으로
        // Object 타입을 사용했었음
        // 이런경우, 반환된 Object 객체를 다시 원하는 타입으로 변환해야하고
        // 운이 나쁜경우, 실행시 오류가 발생할 수도 있었음
...
//        System.out.println((int)numbers3[0] + (int)numbers3[1] + (int)numbers3[2]);
        ...
        // 야생동물 3마리를 동물원에 가둡시다!
        //ArrayList<Lion> zoo2 = new ArrayList<Lion>(); 밑에꺼랑 같음
...
//        zoo2.add(new Tiger());
        ...
        // num2라는 배열변수에는 String 타입 값만 저장할 수 있음
...
class Lion {
    public void sayName(){
        System.out.println("저는 사실 개입니다...");
    }
}
class Tiger {
    public void sayName(){
        System.out.println("저는 사실 고양이입니다...");
    }
}
class Zebra {
    public void sayName(){
        System.out.println("저는 사실 당나귀입니다...");
    }
}
