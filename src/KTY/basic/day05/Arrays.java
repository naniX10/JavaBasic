...
public class Arrays {
	public static void main(String[] args) {
		...
		// 배열을 구성하는 각 값들은 요소라고 하고
		// 배열생성시 몇 개의 요소를 저장할 것인지 갯수를 미리 지정해야 함
		// 헤딩 요소의 위치는 index라는 숫자값으로 지정
		// 배열의 index는 0부터 시작함
		// 1차원, 2차원, 3차원 배열까지 생성 가능

		// 자료형[] 배열변수명; <- 추천!! ex) int[]number = {...};
		// 자료형 배열변수명[];
		...
		uid = new String[3]; // 3명분의 아이디를 저장할 수 있는 배열변수 생성
								...
								// 기본 자료형 객체는 변수에 실제값을 저장함
								// 참조 자료형 객체는 변수에 그 값에 대한 위치값이 저장됨
								// 따라서, 실제값을 알아내려면 위치값 + 변위를 적용해야함
		pwd = new String[3]; // 3명분의 비밀번호를 저장할 수 있는 배열변수 생성
		...
		// 참조형 변수를 출력하면 그 변수값이 저장된 곳의 시작주소가 출력됨
		...
		// System.out.println(uid[1]); <<<< 기본형
		// System.out.println(uid[2]);

		for (int i = 0; i < 3; i += 1) { // <<<<<< 반복문 사용한 심화형
		...
		// nextInt(바운더리) : 0 ~ 경계-1 범위의 난수 생성
		// nextInt(45) -> 0 ~ 44 // 1 더해줘야 1 ~ 45 가능
